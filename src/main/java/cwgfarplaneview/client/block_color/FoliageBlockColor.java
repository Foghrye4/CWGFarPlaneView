...
public class FoliageBlockColor extends BlockColor {

	public FoliageBlockColor(int topSideBlockColorIn, int horizontalSidesBlockColorIn, int bottomSideBlockColorIn) {
	...
	protected int getTopColor(Biome biome, BlockPos pos) {
		if (biome.isSnowyBiome() || biome.getTemperature(pos) < 0.15f)
			return SNOW_COLOR;
		return multiplyColors(topSideBlockColor,biome.getFoliageColorAtPos(pos));
	}
	
	@Override
	protected int getSideColor(Biome biome, BlockPos pos) {
		return multiplyColors(horizontalSidesBlockColor,biome.getFoliageColorAtPos(pos));
	}
