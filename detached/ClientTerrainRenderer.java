...
public class ClientTerrainRenderer extends IRenderHandler implements IBlockAccess {
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		int playerChunkPosX = ((int) renderPosX) >> 4;
		int playerChunkPosZ = ((int) renderPosZ) >> 4;
		...
		if (this.needUpdate)
			compileTerrainDisplayList(world, playerChunkPosX, playerChunkPosZ);
	...
	private void compileTerrainDisplayList(WorldClient world, int playerChunkPosX, int playerChunkPosZ) {
		...
		renderTerrain(world, playerChunkPosX, playerChunkPosZ);
		GL11.glEndList();
		this.needUpdate = false;
	}

	private void renderTerrain(WorldClient world, int playerChunkPosX, int playerChunkPosZ) {
		Tessellator tessellator = Tessellator.getInstance();
		BufferBuilder worldRendererIn = tessellator.getBuffer();
		worldRendererIn.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX_LMAP_COLOR);
		int x0 = Math.max(playerChunkPosX - HORIZONT_DISTANCE, minimalX);
		int z0 = Math.max(playerChunkPosZ - HORIZONT_DISTANCE, minimalZ);
		int x1 = Math.min(maximalX, playerChunkPosX + HORIZONT_DISTANCE);
		int z1 = Math.min(maximalZ, playerChunkPosZ + HORIZONT_DISTANCE);
		for (int x = x0; x <= x1; x++) {
			for (int z = z0; z <= z1; z++) {
